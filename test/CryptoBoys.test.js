...
const CryptoNFTs = artifacts.require("./CryptoNFTs.sol");
...
contract("Crypto NFTs", async (accounts) => {
  let cryptoNFTs, result, cryptoNFTsCount;
  ...
  describe("application features", async () => {
    it("allows users to mint ERC721 token", async () => {
      cryptoNFTsCount = await cryptoNFTs.cryptoNFTsCounter();
      assert.equal(cryptoNFTsCount.toNumber(), 0);
      ...
      let tokenNameExists;
      tokenNameExists = await cryptoNFTs.tokenNameExists("myCBNFT", {
        from: accounts[0],
      });
      ...
      result = await cryptoNFTs.mintCryptoNFTs(
        "myCBNFT",
        "https://gateway.pinata.cloud/ipfs/QmYFmJgQGH4uPHRYN15Xdv4aLd9o4Aq63y1e4GgN6kj5aK/2",
      ...
      cryptoNFTsCount = await cryptoNFTs.cryptoNFTsCounter();
      assert.equal(cryptoNFTsCount.toNumber(), 1);
      ...
      let cryptoNFTs;
      cryptoNFTs = await cryptoNFTs.allCryptoNFTs(1, {
        from: accounts[0],
      });
      assert.equal(cryptoNFTs.tokenId.toNumber(), 1);
      assert.equal(cryptoNFTs.tokenName, "myCBNFT");
      assert.equal(
        cryptoNFTs.tokenURI,
        "https://gateway.pinata.cloud/ipfs/QmYFmJgQGH4uPHRYN15Xdv4aLd9o4Aq63y1e4GgN6kj5aK/2"
      );
      assert.equal(cryptoNFTs.mintedBy, accounts[0]);
      assert.equal(cryptoNFTs.currentOwner, accounts[0]);
      assert.equal(
        cryptoNFTs.previousOwner,
        0x0000000000000000000000000000000000000000
      );
      assert.equal(web3.utils.fromWei(cryptoNFTs.price, "ether"), 1);
      assert.equal(cryptoNFTs.numberOfTransfers.toNumber(), 0);
      assert.equal(cryptoNFTs.forSale, true);
      ...
      await cryptoNFTs.mintCryptoNFTs(
        "myCBNFT2",
    ...
    it("allows users to buy token for specified ethers", async () => {
      ...
      assert.equal(cryptoNFTs.numberOfTransfers.toNumber(), 1);
      ...
      assert.equal(cryptoNFTs.currentOwner, accounts[2]);
    ...
    it("allows users to change token price", async () => {
      let cryptoNFTsPrice;
      cryptoNFTsPrice = await cryptoNFTs.allCryptoNFTs(1, {
        from: accounts[0],
      });
      assert.equal(web3.utils.fromWei(cryptoNFTsPrice.price, "ether"), 1);
      ...
      assert.equal(web3.utils.fromWei(cryptoNFTsPrice.price, "ether"), 2);
    ...
    it("allows users to toggle between setting the token for sale or not for sale", async () => {
      ...
      assert.equal(cryptoNFTs.forSale, false);
